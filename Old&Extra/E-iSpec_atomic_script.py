...
if np.any(index):
    ...
    initial_teff = 6000.0; initial_logg = 1.0; initial_MH = -1.5; initial_vmic = 2.00
initial_R = 85000.
...
def ContFitAndNorm(star_spectrum):
    ...
    # Strategy: start with median filtering and continue with maximum filtering in order to find the continuum
    order='median+max'
...
def RVCorr(star_spectrum):
    ...
    rv = models[0].mu(); rv_err = models[0].emu() # km/s
...
def ListCreation(wave_base, wave_top):
    ...
    # To account for some expected variations in elemental abundances (manual change)
    ...
    new_atomic_linelist = ispec.calculate_theoretical_ew_and_depth(atmosphere_layers, \
            initial_teff, initial_logg, initial_MH, alpha, \
            atomic_linelist, isotopes, solar_abundances, microturbulence_vel=initial_vmic, \
            verbose=1, gui_queue=None, timeout=900)
    #new_atomic_linelist = new_atomic_linelist[new_atomic_linelist['theoretical_ew']>5.] # theoretical EW cut, if needed
...
def LineFit(star_spectrum, star_continuum_model, rv, mode='seek'):
    ...
    vel_telluric = -rv; vel_telluric_err = rv_err # We start with spectra which are not yet corrected for RV

    # Line regions
    if mode=='seek':
        smooth_spectrum = ispec.convolve_spectrum(star_spectrum, 50000., from_resolution=initial_R); ID = 'element'
        #####--- Find linemasks ------------------------------------------------
        line_regions = ispec.find_linemasks(smooth_spectrum, star_continuum_model,\
    ...
    if mode=='tweak' or mode=='pick':
        if objName=='J065127':
            #line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "SZMon_gen_cool.txt"); ID = 'note'
            line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "SZMon_gen_hot.txt"); ID = 'note'
        elif objName=='J194853':
            #line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "DFCyg_gen_cool_old.txt"); ID = 'note'
            #line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "DFCyg_gen_cool.txt"); ID = 'note'
            #line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "DFCyg_gen_cool_new2.txt"); ID = 'note'
            #line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "DFCyg_gen_cool_new3.txt"); ID = 'note'
            #line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "DFCyg_gen_co.txt"); ID = 'note'
            line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "DFCyg_gen_hot.txt"); ID = 'note'
            #line_regions = ispec.read_line_regions(ispec_dir + mySamIn_dir + "DFCyg_gen_ho.txt"); ID = 'note'
...
def LineFitPlot(star_spectrum, linemasks, mode):
    ...
    for i in range(len(linemasks)):
        ...
        plt.xlim([spec['waveobs'][0]+0.1,spec['waveobs'][-1]-0.1])
        plt.ylim([0.2,1.2])
...
def EWparam(star_spectrum, star_continuum_model, linemasks, code='moog', mode='default'):
    ...
    if not ispec.valid_atmosphere_target(modeled_layers_pack, {'teff':initial_teff, 'logg':initial_logg, 'MH':initial_MH, 'alpha':initial_alpha}):
        ...
        print(msg)
    # Filter too weak/strong lines based on reduced equivalent width
...
def EWabund(star_spectrum, star_continuum_model, linemasks, code='moog', mode='default'):
    ...
    if initial_MH < -2.5:
        initial_MH = -2.5
    #--- Manually tweaking atmospheric parameters for estimation of elemental abundance errors
    #initial_teff += 70 #40.
    #initial_logg += .09 #.14
    #initial_MH += .05 #.1
    #initial_vmic += .03 #.05
    ...
    if mode=='ssfl2l':
        ...
        atomic_linelist_file = ispec_dir+mySamOut_dir+objName+"_LineList.txt"
        atomic_linelist = ispec.read_atomic_linelist(atomic_linelist_file, wave_base=np.min(star_spectrum['waveobs']), wave_top=np.max(star_spectrum['waveobs']))
        #atomic_linelist = atomic_linelist[atomic_linelist['theoretical_depth'] >= 0.01] # if needed
        chemical_elements_file = ispec_dir + "input/abundances/chemical_elements_symbols.dat"
        chemical_elements = ispec.read_chemical_elements(chemical_elements_file)
...
def StepReduc(objName):
    if objName=='J065127':
        #star_spectrum = ispec.read_spectrum("/home/max/CallofPhDuty/Spectra/OPTICdata/SZMon/SZMon_norm_00458609.txt") #cool (main)
        #star_spectrum = ispec.read_spectrum("/home/max/CallofPhDuty/Spectra/OPTICdata/SZMon/SZMon_norm_00393045.txt") #cool (new)
        star_spectrum = ispec.read_spectrum("/home/max/CallofPhDuty/Spectra/OPTICdata/SZMon/SZMon_norm_00866282.txt") #hot (main)
        #star_spectrum = ispec.read_spectrum("/home/max/CallofPhDuty/Spectra/OPTICdata/SZMon/SZMon_norm_00397261.txt") #hot (test)
    elif objName=='J194853':
        #star_spectrum = ispec.read_spectrum("/home/max/CallofPhDuty/Spectra/OPTICdata/DFCyg/DFCyg_norm_00560174.txt") #cool (main)
        star_spectrum = ispec.read_spectrum("/home/max/CallofPhDuty/Spectra/OPTICdata/DFCyg/DFCyg_norm_00972481.txt") #hot (main)
        #star_spectrum = ispec.read_spectrum("/home/max/CallofPhDuty/Spectra/OPTICdata/DFCyg/DFCyg_norm_00412205.txt") #hot (test)
...
if __name__ == '__main__':
    star_spectrum, star_continuum_model, rv, rv_err = StepReduc(objName) # To prepare spectra for chemical analysis
    #linemasks = StepFind(star_spectrum, star_continuum_model, rv) # To come up with selected lines
    #linemasks = StepFilter(star_spectrum, star_continuum_model, rv) # To manually filter automatically selected lines
    linemasks = StepStud(star_spectrum, star_continuum_model, rv) # To further fine-tune the filtered line list
    pass

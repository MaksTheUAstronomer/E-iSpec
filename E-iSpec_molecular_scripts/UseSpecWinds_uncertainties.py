...
def O_Initial(objName, testName, sigRead=1, delRead=1., code='etom'):
    ...
    if code=='ASPCAP':
        ...
        skiprows=0
    if not os.path.exists(iSpecFol+"mySample/WindowScripts/Abunds/Abunds_%s_%s/" % (objName, testName)):
        os.makedirs(iSpecFol+"mySample/WindowScripts/Abunds/Abunds_%s_%s/" % (objName, testName))
...
def O_CalcChi2(fl0, fl, spw):
    dum = (fl[spw!=0.]-fl0[spw!=0.])/np.nanstd(fl[spw!=0.]); chi2 = np.sum(dum**2) #np.sum(spw[spw!=0.]/np.sum(spw)*dum**2) #*spw
    return(chi2)
...
def O_IsoRatioRange(species):
    if species=='13C':
        isoRinit = 2.; isoRfin = 92.5
    elif species=='15N':
        isoRinit = 2.; isoRfin = 271.
    elif species=='17O':
        isoRinit = 2.; isoRfin = 2630.
    elif species=='18O':
        isoRinit = 2.; isoRfin = 487.
    return(isoRinit, isoRfin)
...
def V_ElemOrder(objName, testName, species, spec, SpecWinds, colNames): # Defines order of chemical elements for abundance calculation
    updS = []; updA = []; updD = []
    teff, logg, met, vmic, vsini = O_ReadParams(testName)
    #--- Fe ---------------------------------------------------------------
    ...
    #--- CNO --------------------------------------------------------------
    updS, updA, updD, order = V_ElemOrder_CNOiter(objName, testName, updS, updA, updD, met) # V_ElemOrder_CNOiter(objName, testName, updS, updA, updD, met)
    ...
    mask = (species!='C') & (species!='N') & (species!='O') & (species!='Fe') & (species!='He') # Why He gives an error in 'modeling/ssf.py' (KeyError: '2')?
...
def V_ElemOrder_NOiter(objName, testName, updS, updA, updD, met): # In case there are atomic lines only of C
    precisC = 0.01; oldC = 0.; newC = 8.43+met
    precisN = 0.01; oldN = 0.; newN = 7.83+met
    precisO = 0.01; oldO = 0.; newO = 8.69+met
    iterC=0; iterN=0; iterO=0; order=[]
...
def V_CheckSpecWinds(objName, testName, species, spec, SpecWinds, colNames, speciesType='elem', updS=[], updA=[]):
    with open("Abunds/Abunds_%s_%s/%s_SynthLines%s_%s.txt" % (objName, testName, objName, testName, species), "w") as outfile:
        outfile.write('Wave_low\twave_upp\tAbund\tchi-2\tSp.w. peak\n')
        if np.all(SpecWinds[colNames[species]]==0):
            outfile.write('-\t-\t-\t-\t-\n')
            newAb = O_MetScalSolAbund(testName, species); delAb = -1.
        else:
...
def V_Calc1Species(objName, testName, species, spec, SpecWinds, colNames, outfile, speciesType, updateSpecie=[], updateAbund=[]):
    abunds = []; teff, logg, met, vmic, vsini = O_ReadParams(testName)
    pdf = PdfPages("Abunds/Abunds_%s_%s/%s_SynthLines%s_%s.pdf" % (objName, testName, objName, testName, species))
    ...
    for patch in nonzeros:
        ...
        if speciesType=='elem':
            abCntr = I_IterateFib(objName, testName, cwl, species, spec0, spw, updateSpecie, updateAbund)[0]
            specL,specM,specU = V_SynthSpecTriplet(objName, testName, cwl, abCntr, spw, colNames, species, speciesType=speciesType, updateSpecie=updateSpecie, updateAbund=updateAbund)
            ...
            isoR = II_IterateFib(objName, testName, cwl, species, spec0, spw, updateSpecie, updateAbund)
            if updateAbund:
                abund = updateAbund[updateSpecie.index(species[-1])]
            else:
                abund = O_MetScalSolAbund(testName, species[-1])
            specL,specM,specU = V_SynthSpecTriplet(objName, testName, cwl, abund, spw, colNames, species, isoR, speciesType=speciesType, updateSpecie=updateSpecie, updateAbund=updateAbund)
            chi2 = [O_CalcChi2(spec0[1],specZ[1],spw[1]), O_CalcChi2(spec0[1],specL[1],spw[1]), O_CalcChi2(spec0[1],specM[1],spw[1]), O_CalcChi2(spec0[1],specU[1],spw[1])]
...
def V_SynthSpecTriplet(objName, testName, cwl, abCntr, SpecWinds, colNames, species, isoR=1., speciesType='elem', updateSpecie=[], updateAbund=[]):
...
def I_IterateFib(objName, testName, cwl, elem, spec0, SpecWinds, updateSpecie=[], updateAbund=[]):
    teff, logg, met, vmic, vsini = O_ReadParams(testName)
    abund = O_MetScalSolAbund(testName, elem)
...
def II_IterateFib(objName, testName, cwl, species, spec0, SpecWinds, updateSpecie=[], updateAbund=[]):
    teff, logg, met, vmic, vsini = O_ReadParams(testName)
    if updateAbund:
        abund = updateAbund[updateSpecie.index(species[-1])]
    else:
    ...
    n = 12
    if species=='13C':
        a = 1.5; fibon = O_Fib(n)*1. # From 1 to 145 (solar value ~92.5)
    elif species=='15N':
        a = 1.5; fibon = O_Fib(n)*4. # From 1 to 577 (solar value ~271)
    elif species=='17O':
        a = 1.5; fibon = O_Fib(n)*4. # From 1 to 577 (solar value ~2630)
    elif species=='18O':
        a = 1.5; fibon = O_Fib(n)*4. # From 1 to 577 (solar value ~487)
    b = a+fibon
